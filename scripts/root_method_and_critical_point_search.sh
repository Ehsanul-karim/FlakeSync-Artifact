...
while IFS= read -r line
    ...
    root_method_phase_start=$(phase_now_ns)
    ...
    rootLine=""
    if [[ ! -d ${inputProj}/${slug}/.git ]]; then
        mkdir -p "${inputProj}/${rootProj}"
    ...
    echo "#Project-Name,SHA,Module,Test-Name,Thred-ID,Location[delay],Time" >> "$currentDir/$result_file_name"
    ...
    if [[ $len -gt 0 ]]; then # Now printing each array item into the outputFile
        ...
        mvn test $JMVNOPTIONS -pl $module -Dtest=$testName -DsearchForMethodName="$currentDir/logs/org_loc.txt" > "$currentDir/logs/search-for-method-name-$tt_file" 
...
root_method_phase_end=$(phase_now_ns)
critical_line_phase_start=$(phase_now_ns)
bash analyzeRootMethod.sh "$currentDir/${result_file_name}" "Results-Boundary/Boundary-${tt_file}-Result.csv"
critical_line_rc=$?
critical_line_phase_end=$(phase_now_ns)

root_method_phase_sec=$(phase_seconds_between "$root_method_phase_start" "$root_method_phase_end")
critical_line_phase_sec=$(phase_seconds_between "$critical_line_phase_start" "$critical_line_phase_end")

echo "FLAKESYNC_STAGE2_TEST=$testName"
echo "FLAKESYNC_ROOT_METHOD_SECONDS=$root_method_phase_sec"
echo "FLAKESYNC_CRITICAL_LINE_SECONDS=$critical_line_phase_sec"
echo "FLAKESYNC_CRITICAL_LINE_EXIT_CODE=$critical_line_rc"
done < $1    
