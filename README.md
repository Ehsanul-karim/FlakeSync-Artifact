...
This readme is just a replica of official readme from FlakeSync. I just run the artifact on two samples form alltests.csv and store the results here. 
...
The artifact needs to be run from Docker. After downloading the Docker image `flakesync-artifact\_latest.tar.gz` from the link, you can install the image by the following command:

```shell
docker load < flakesync-artifact\_latest.tar.gz
```
...
bash end\_to\_end\_flakesync.sh data\_list/input.csv
```

Here, `data\_list/input.csv` contains the line `alibaba/wasp,b2593d8,.,com.alibaba.wasp.executor.TestExecutorService#testExecutorService`, meaning the command will run FlakeSync on the test `com.alibaba.wasp.executor.TestExecutorService#testExecutorService` from project `alibaba/wasp` on commit `b2593d8`.
...
`com.alibaba.wasp.executor.TestExecutorService#testExecutorService\_Actual\_Location.csv` contains similar information, with the contents of the file looking something like:
...
```shell
cd /home/java8-flakesync/scripts/
bash delay\_injection\_and\_minimized\_locations.sh data\_list/input.csv
```
...
```shell
cd /home/java8-flakesync/scripts/
bash root\_method\_and\_critical\_point\_search.sh Results-Minimizer/com.alibaba.wasp.executor.TestExecutorService#testExecutorService\_Actual\_Location.csv output
```
...
```shell
cd /home/java8-flakesync/scripts/
bash barrier\_point\_search.sh Results-Boundary/Boundary-com.alibaba.wasp.executor.TestExecutorService#testExecutorService-Result.csv
```
